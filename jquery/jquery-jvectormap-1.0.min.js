...
jvm.AbstractShapeElement.prototype.setSelected = function(isSelected){
  if (this.isSelected !== isSelected) {
    ...
    this.updateStyle();
    jQuery(this.node).trigger('selected', [isSelected]);
  }
};
...
jvm.AbstractShapeElement.prototype.updateStyle = function(){
  ...
  jQuery.extend(attrs, this.style.initial || {});
  jQuery.extend(attrs, this.style.current || {});
  if (this.isHovered) {
    jQuery.extend(attrs, this.style.hover || {});
  }
  if (this.isSelected) {
    jQuery.extend(attrs, this.style.selected || {});
    if (this.isHovered) {
      jQuery.extend(attrs, this.style.selectedHover || {});
    }
  }
  this.set(attrs);
};
...
jvm.VMLElement.prototype.addClass = function( className ){
  jQuery(this.node).addClass(className);
};
...
jvm.VMLPathElement.pathSvgToVml = function(path) {
  ...
  return path.replace(/([MmLlHhVvCcSs])\s*((?:-?\d*(?:\.\d+)?\s*,?\s*)+)/g, function(segment, letter, coords, index){
    coords = coords.replace(/(\d)-/g, 'jQuery1,-')
            .replace(/^\s+/g, '')
            .replace(/\s+jQuery/g, '')
            .replace(/\s+/g, ',').split(',');
...
};// IPV Edit - change all "$" to "jQuery" so we can use CMS builtins
/**
...
jvm.WorldMap = function(params) {
  ...
  this.params = jQuery.extend(true, {}, jvm.WorldMap.defaultParams, params);
...
jvm.WorldMap.prototype = {
  ...
  reset: function() {
    ...
    //IPV edit - next line is a bug in jvectomap, we'll just comment it out
  ...
  applyTransform: function() {
	...
	// IPV Edit:  allow a buffer so we can center countries near the 
	// "edge of the earth" when we setFocus to them
	
	var c_buff = 300;

    var maxTransX, maxTransY, minTransX, minTransY;
    if (this.defaultWidth * this.scale <= this.width) {
      maxTransX = (this.width - this.defaultWidth * this.scale) / (2 * this.scale) + c_buff;
      minTransX = (this.width - this.defaultWidth * this.scale) / (2 * this.scale) - c_buff;
    } else {
      maxTransX = c_buff;
      minTransX = (this.width - this.defaultWidth * this.scale) / this.scale - c_buff;
    }

    if (this.defaultHeight * this.scale <= this.height) {
      maxTransY = (this.height - this.defaultHeight * this.scale) / (2 * this.scale) + c_buff;
      minTransY = (this.height - this.defaultHeight * this.scale) / (2 * this.scale) - c_buff;
    } else {
      maxTransY = c_buff;
      minTransY = (this.height - this.defaultHeight * this.scale) / this.scale - c_buff;
    }
  ...
  bindContainerTouchEvents: function(){
    ...
    jQuery(this.container).bind('gesturestart', function(e){
      touchStartScale = map.scale;
      touchStartTransX = map.transX;
      touchStartTransY = map.transY;
      return false;
    });
    jQuery(this.container).bind('gesturechange', function(e){
  ...
  bindElementEvents: function(){
    ...
    this.container.delegate('.jvectormap-region, .jvectormap-marker', 'mouseover mouseout', function(e){
      var path = this,
          type = jQuery(this).attr('class').indexOf('jvectormap-region') === -1 ? 'marker' : 'region',
          ...
          labelShowEvent = jQuery.Event(type+'LabelShow.jvectormap'),
  ...
  createLabel: function(){
    ...
    this.label = jQuery('<div/>').addClass('jvectormap-label').appendTo(jQuery('body'));
  ...
  setFocus: function(scale, centerX, centerY){
    ...
    if (typeof scale === 'string' && this.regions[scale]) {
	  ...
	  // IPV EDIT - if scale is actually a region code, treat centerX as the 
	  // buffer percent to show neighboring countries
	 
	  if ( typeof centerX === "undefined" ) centerX = 1.0;

      bbox = this.regions[scale].element.getBBox();
      this.setScale(
        Math.min( centerX * this.width / bbox.width, centerX * this.height / bbox.height),
        - (bbox.x + bbox.width / 2),
  ...
  createRegions: function(){
    ...
    for (key in this.mapData.paths) {
      region = this.canvas.addPath({
        ...
        "data-code": key
      }, jQuery.extend({}, this.params.regionStyle));
      jQuery(region.node).bind('selected', function(e, isSelected){
        map.container.trigger('regionSelected.jvectormap', [jQuery(this).attr('data-code'), isSelected, map.getSelectedRegions()]);
      });
  ...
  createMarkers: function(markers) {
    ...
    for (i in markers) {
      ...
      marker = this.canvas.addCircle({
        ...
        cx: point.x,
        cy: point.y
      }, jQuery.extend(true, {}, this.params.markerStyle, {initial: markerConfig.style || {}}), group);
      marker.addClass('jvectormap-marker');
      jQuery(marker.node).bind('selected', function(e, isSelected){
        map.container.trigger('markerSelected.jvectormap', [jQuery(this).attr('data-index'), isSelected, map.getSelectedMarkers()]);
      });
      if (this.markers[i]) {
